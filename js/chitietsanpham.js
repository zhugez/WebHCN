...
function phanTich_URL_chiTietSanPham() {
    ...
    var s = addThongSo('Cảm biến', sanPham.detail.manHinh);
    s += addThongSo('Màn hình', sanPham.detail.os);
    s += addThongSo('Lấy nét tự động', sanPham.detail.camara);
    s += addThongSo('Quay phim', sanPham.detail.camaraFront);
    s += addThongSo('ISO', sanPham.detail.cpu);
    s += addThongSo('Tốc độ chụp', sanPham.detail.ram);
    s += addThongSo('Bộ nhớ', sanPham.detail.rom);
    s += addThongSo('Kích thước', sanPham.detail.microUSB);
    s += addThongSo('Trọng lượng', sanPham.detail.battery);
    info.innerHTML = s;
